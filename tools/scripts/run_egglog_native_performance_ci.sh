...
if [ -n "${D810_REPO_ROOT:-}" ]; then
  CORE_ROOT="$(cd "$D810_REPO_ROOT" && pwd)"
else
  echo "ERROR: D810_REPO_ROOT must point to the core checkout" >&2
  exit 1
fi
...
for MODE in 1 0; do
  ...
  if [ "$CPROFILE" = 1 ]; then
    case "$ARTIFACT_DIR" in
      "$WORKTREE_DIR"/*)
        ...
        CONTAINER_PROFILE_DIR="/work/${PROFILE_HOST_DIR#"$WORKTREE_DIR/"}"
        ;;
      *)
  ...
  : > "$LOG"
  D810_REPO_ROOT="$CORE_ROOT" D810_EGGLOG_ROOT="$EXTENSION_ROOT" \
...
"${PYTHON:-python3}" "$EXTENSION_ROOT/tools/scripts/compare_egglog_native_performance_receipts.py" \
