...
from d810_egglog import saturation as egglog_saturation
...
_ROOT_OPCODE_BY_OPERATION = MappingProxyType(
    {
        operation: operation
        for operation in ("add", "and", "bnot", "mul", "neg", "or", "sub", "xor")
    }
)
...
@dataclass(frozen=True)
class _RuleSpecialization:
    """AST inspection result for one admitted native rewrite."""

    rule: object
    candidate_ast: object
...
def specialize(rule, ast, *, destination_size: int, rounds: int = 2):
    ...
    try:
        ...
        for _matched_rule, replacement_term, _declaration_index in applications:
            if term_cost(replacement_term) >= term_cost(candidate.term):
                continue
            replacement_ast = native_mba_host_services().rebuild_ast(
                candidate,
                replacement_term,
            )
...
class EgglogOptimizer(PeepholeSimplificationRule):
    ...
    def __init__(self) -> None:
        super().__init__()
        self._host = native_mba_host_services()
        # Keep the original handler default (MMAT_GLBOPT2) behind the host
        # adapter.  A configuration may override it later, but omitted config
        # must retain the production maturity gate.
        self.maturities = list(self._host.maturities_for_names(["GLOBAL_OPTIMIZED"]))
    ...
    def configure(self, kwargs) -> None:
        ...
        if maturity_names is not None:
            try:
                self.maturities = list(self._host.maturities_for_names(maturity_names))
    ...
    def _create_composite_cache(self) -> EgglogIdbCompositeCache:
        ...
        return EgglogIdbCompositeCache(
            self._host.persistence(EgglogIdbCompositeCache.NAMESPACE),
        ...
        *,
        ...
        if applications:
            candidates.extend(
                (
                    replacement,
                    self._provenance_from_rule(rule),
                )
                for rule, replacement, _declaration_index in applications
            )
        elif structural_route and structural_matches:
            candidates.extend(
    ...
    def _finish_extraction_candidate(
        ...
        *,
            ...
            try:
                if candidate_ast is not None:
                    candidate = self._host.capture_ast(
                        candidate_ast, destination_size=destination_size
                    )
                else:
                    candidate = self._host.capture_instruction(ins)
            except Exception:
                candidate = None
        if candidate is None:
            if allow_fallback:
            ...
            return None
        expected_term = (
            candidate.term if expected_lowering_term is None else expected_lowering_term
        )
        if candidate.term != expected_term:
            if allow_fallback:
                self._mark_replay_fallback("rebuild_failed")
                return None
            self._record_extraction_receipt(
        ...
        try:
            try:
                reconstruction = self._host.rebuild(candidate, replacement_term)
            except Exception:
                reconstruction = None
        finally:
        ...
        try:
            (
                ...
                replacement_term=replacement_term,
                selected=selected,
                width=int(candidate.destination_size) * 8,
        ...
        try:
            new_ins = reconstruction.replacement_instruction
        finally:
    ...
    def _check_and_replace(self, ins, *, blk=None):
        """Run the provider-owned pipeline through the native host facade."""
        ...
        if destination is None:
            self._record_extraction_receipt(
                EgraphExtractionReceipt(skip_reason=EgraphSkipReason.NON_MBA_CANDIDATE)
            )
            return None
        if type(getattr(destination, "size", None)) is not int:
            self._record_extraction_receipt(
                EgraphExtractionReceipt(
        ...
        try:
            ...
            if candidate is None:
                ...
                return None
            candidate = self._host.prepare_cross_block(
                candidate,
                block=blk,
                instruction=ins,
                use_constants=self.cross_block_constant_preparation,
                use_def_use=self.cross_block_def_use_preparation,
            )
            ...
            candidate_skip_reason = self._native_view_skip_reason(profile)
            if candidate_skip_reason is not None:
                self._record_extraction_receipt(
                    extraction_receipt_for_profile(profile, candidate_skip_reason)
                )
                return None
        ...
        if extraction_budget is None:
            self._record_extraction_receipt(
                extraction_receipt_for_profile(profile, EgraphSkipReason.TIME_BUDGET)
            )
            return None
        ...
        if direct_application is not None:
            ...
            return self._finish_extraction_candidate(
                ...
                profile=profile,
                telemetry_match_result=matcher_telemetry,
        ...
        try:
            ...
            extraction = self._run_fresh_saturation(candidate_term, **extraction_kwargs)
        finally:
    ...
    def _select_specialization(
    ...
    ) -> _RuleSpecialization | None:
        """Select one strict host-rebuilt application for AST inspection."""
        self._ensure_catalogue_configured()
        ...
        try:
            ...
            return None
        root_operation = candidate.term.operation
        if root_operation is None:
            return None
        best: _RuleSpecialization | None = None
        best_cost: int | None = None
        for rule in self._rules_by_root_opcode.get(root_operation, ()):
            ...
            if self._node_count(specialization.replacement_ast) >= self._node_count(
                ...
                continue
            if width not in getattr(rule, "proof_widths", ()):
                continue
            if not self._host.prove_ast(
                candidate,
    ...
    def _build_root_opcode_buckets(
        ...
        buckets: dict[str, list[CompiledMbaRule]] = {}
        for rule in rules:
            operation = getattr(getattr(rule, "pattern", None), "operation", None)
            if operation is None and self.families == _DEFAULT_FAMILIES:
                # Preserve the ADD-only selection behavior for synthetic rules
                # whose test doubles have no DSL pattern.
                operation = "add"
            if operation not in _ROOT_OPCODE_BY_OPERATION:
                raise ValueError(
                    f"Egglog rule {rule.source_name} has unsupported root operation "
                    f"{operation!r}"
                )
            buckets.setdefault(operation, []).append(rule)
        return MappingProxyType(
            {operation: tuple(bucket) for operation, bucket in buckets.items()}
        )
    ...
    def _node_count(node) -> int:
        if node is None:
            return 0
        if (
            getattr(node, "opcode", None) is not None
            and getattr(node, "d", None) is not None
            and not callable(getattr(node, "is_leaf", None))
        ):
            return 1
        if callable(getattr(node, "is_leaf", None)) and node.is_leaf():
            return 1
        return (
            1
    ...
    def _prove_selected_replacement(
        ...
        *,
        ...
        """Use the host's current native proof for every candidate."""

        del width
        template_verdict = None
        template_elapsed_ms = None
        if self.native_proof_mode == "shadow":
            template_started = time.perf_counter()
            try:
                template_verdict = prove_typed_term_equivalence(
                    original_term,
                    replacement_term,
                )
            except Exception:
                template_verdict = False
            template_elapsed_ms = (time.perf_counter() - template_started) * 1000.0
        started = time.perf_counter()
        try:
            proved = self._host.prove(
                candidate,
                reconstruction,
        ...
        fallback_reason = (
            "shadow_divergence"
            if self.native_proof_mode == "shadow" and template_verdict != proved
            else None
        )
        accepted = proved and fallback_reason is None
        return (
            accepted,
            selected[1],
            fallback_reason,
