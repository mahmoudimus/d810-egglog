...
def test_shared_guarded_add_rewrites_fit_the_64_eclass_admission_cap():
    ...
    assert host.rebuild_ast(native_candidate, result.replacement_term) is not None
...
def test_live_handler_xor_root_never_specializes_unrelated_root_buckets(monkeypatch):
    ...
    monkeypatch.setattr(
        "d810_egglog.rules.egglog_optimizer.compiled_rules_for_families",
        select_rules,
    )
...
def test_central_statistics_records_selected_family_provenance(monkeypatch):
    ...
    captured = real_host.capture_ast(candidate, destination_size=4)
    ...
    class _SyntheticHost:
        ...
        def rebuild(self, _candidate, _replacement):
            return NativeMbaReconstruction(candidate, _Instruction())

    handler._host = _SyntheticHost()
