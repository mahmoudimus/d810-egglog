...
def test_specialize_accepts_semantically_cloned_catalogue_rule(monkeypatch):
    rule = _catalogue_rule("Add_HackersDelightRule_2")
    source_term = TypedBvTerm(
        "add",
        32,
        children=(
            TypedBvTerm(None, 32, leaf_key=("register", 1)),
            TypedBvTerm(None, 32, value=1),
        ),
    )
    replacement_term = TypedBvTerm(None, 32, leaf_key=("register", 1))

    class FakeHost:
        def capture_ast(self, _ast, *, destination_size):
            assert destination_size == 4
            return SimpleNamespace(term=source_term)

        def rebuild_ast(self, _candidate, replacement):
            assert replacement is replacement_term
            return SimpleNamespace(opcode="leaf")
    ...
    monkeypatch.setattr(
        ...
        lambda _rules: FakeCatalogue(),
    )

    specialization = egglog_optimizer.specialize(
        rule,
        object(),
        destination_size=4,
    )
    ...
    assert specialization is not None
    assert specialization.rule is rule
...
def test_live_handler_selects_later_certified_specialization_with_lower_cost(
    ...
    monkeypatch.setattr(
        "d810_egglog.rules.egglog_optimizer.specialize",
