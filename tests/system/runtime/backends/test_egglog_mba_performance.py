...
from d810.mba.extension_api import NativeMbaReconstruction  # noqa: E402
...
def test_corpus_receipt_reports_quantiles_and_rejects_100x_regression(
    ...
    class _StageHost:
        """Delegate native operations while supplying the active test AST."""

        def __getattr__(self, name):
            return getattr(real_host, name)

        def capture_instruction(self, _instruction):
            return real_host.capture_ast(current_candidate[0], destination_size=4)

        def rebuild(self, candidate, replacement):
            replacement_ast = real_host.rebuild_ast(candidate, replacement)
            if replacement_ast is None:
                return None
            return NativeMbaReconstruction(replacement_ast, object())
    ...
    # The profile is a real handler/extraction/native-proof path over active
    # AST terms. Only callback capture is synthetic; reconstruction and proof
    # remain delegated to the current host services.
    stage_handler._host = _StageHost()
    # Keep the profile comparable with the historical six-stage corpus: force
    # the bounded extraction stage instead of accepting a direct catalogue hit.
    stage_handler._direct_native_application = lambda **_kwargs: None
    ...
    try:
        ...
        # The extension is mounted at /opt/d810-egglog while pytest runs from
        # the core /work tree; retain an absolute artifact identity there.
        baseline_fixture = str(_BASELINE_PATH)
    report.update(
        {
