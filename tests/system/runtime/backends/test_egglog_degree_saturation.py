...
from d810_egglog.structural_rules import (  # noqa: E402
...
from d810_egglog.idb_cache import (  # noqa: E402
...
from d810_egglog.composite_rewrite import (  # noqa: E402
...
_synthetic_ast_converter = minsn_to_ast
...
def _host_fixture_bridge(monkeypatch):
    """Route synthetic instruction conversion through the native host seam."""

    import d810_egglog.rules.egglog_optimizer as handler_module

    real_host = native_mba_host_services()

    class _SyntheticHost:
        def __getattr__(self, name):
            return getattr(real_host, name)

        def capture_instruction(self, ins):
            destination = getattr(ins, "d", None)
            destination_size = getattr(destination, "size", None)
            if type(destination_size) is not int:
                return None
            ast = _synthetic_ast_converter(ins)
            if ast is None:
                return None
            try:
                return real_host.capture_ast(ast, destination_size=destination_size)
...
def _extract_ast_term(candidate, rules, budget, destination_size, **kwargs):
    """Lower an AST in the test and keep saturation typed-term-only."""
...
def _rebuild_ast_from_term(candidate, result, destination_size):
    """Use the native host for test-only AST reconstruction and proof setup."""

    if result.replacement_term is None:
        return None
    try:
        host = native_mba_host_services()
        native_candidate = host.capture_ast(
            candidate,
            destination_size=destination_size,
        )
        return host.rebuild_ast(native_candidate, result.replacement_term)
    except Exception:
        return None
...
class TestCertifiedFixedRotateMaterialization:
    ...
    def test_certified_fixed_rotate_extracts_and_materializes_native_helper(
        ...
        assert lowering.raw_term is not None, lowering.profile
        rules = tuple(
        ...
        assert result.replacement_term is not None, result.receipt
        candidate.dst_mop = output
        host = native_mba_host_services()
        native_candidate = host.capture_ast(candidate, destination_size=8)
        ...
        assert replacement_ast.opcode == ida_hexrays.m_mov
        assert replacement_ast.l.d.opcode == ida_hexrays.m_call
        assert replacement_ast.l.d.l.helper == "__ROL8__"
        ...
        instruction.swap(replacement_ast)
...
@pytest.mark.parametrize(
    "source", _historical_xor_forms(), ids=("subtraction", "negative-coefficient")
)
...
def test_three_ms_telemetry_path_records_path_without_loading_egglog(monkeypatch):
    ...
    monkeypatch.setattr("d810_egglog.saturation._load_egglog_module", forbidden_load)
    result = extract_bounded_term(
        source,
        (),
        EgglogExtractionBudget(),
...
def test_one_catalogue_rewrite_extracts_at_degree_one_and_proves_all_widths(size):
    ...
    replacement_ast = _rebuild_ast_from_term(candidate, result, size)
    ...
    host = native_mba_host_services()
    native_candidate = host.capture_ast(candidate, destination_size=size)
...
def test_active_runtime_proxy_extracts_and_reconstructs_all_widths(size):
    ...
    replacement_ast = _rebuild_ast_from_term(
        ast_dispatcher.AstProxy(candidate), result, size
    )
    assert result.replacement_term is not None
    assert replacement_ast is not None
    assert result.receipt.degree == 1
...
def test_active_cython_proxy_uses_real_c_ast_types(size):
    ...
    replacement_ast = _rebuild_ast_from_term(c_ast.AstProxy(candidate), result, size)
    assert isinstance(replacement_ast, c_ast.AstNode)
    assert result.replacement_term is not None
    assert result.receipt.skip_reason is None
...
def test_direct_ast_composed_catalogue_rewrite_needs_degree_two():
    ...
    degree_one = _extract_ast_term(
        candidate,
        rules,
        EgglogExtractionBudget(max_degree=1, **common),
        4,
    )
    degree_two = _extract_ast_term(
        candidate,
        rules,
    ...
    assert degree_one.replacement_term is None
    assert degree_one.receipt.skip_reason is EgraphSkipReason.NON_MBA_CANDIDATE
    replacement_ast = _rebuild_ast_from_term(candidate, degree_two, 4)
    assert degree_two.replacement_term is not None
    assert replacement_ast is not None
...
def test_ac_operand_order_uses_no_variant_catalogue_rewrite():
    ...
    replacement_ast = _rebuild_ast_from_term(swapped, result, 4)
    assert result.replacement_term is not None
    assert replacement_ast is not None
    assert result.receipt.degree == 1
    assert result.receipt.rule_firings == 1
...
def test_ac_reassociation_uses_no_associativity_rewrite():
    ...
    replacement_ast = _rebuild_ast_from_term(candidate, result, 4)
    assert result.replacement_term is not None
    assert replacement_ast is not None
    assert result.receipt.degree == 1
    assert result.receipt.rule_firings == 1
...
def test_grounded_constant_guard_fires_only_when_constraint_holds():
    ...
    valid = _extract_ast_term(
        valid_candidate,
        (rule,),
        EgglogExtractionBudget(time_budget_ms=1000),
        4,
    )
    invalid = _extract_ast_term(
        candidate(6),
        (rule,),
        EgglogExtractionBudget(time_budget_ms=1000),
        4,
    ...
    replacement_ast = _rebuild_ast_from_term(valid_candidate, valid, 4)
    assert valid.replacement_term is not None
    assert replacement_ast is not None
    ...
    host = native_mba_host_services()
    native_candidate = host.capture_ast(valid_candidate, destination_size=4)
    assert host.prove_ast(
        native_candidate,
        replacement_ast,
        certificate=None,
    ...
    assert invalid.replacement_term is None
    assert invalid.receipt.rule_firings == 0
    assert invalid.receipt.skip_reason is EgraphSkipReason.NON_MBA_CANDIDATE
...
class _DictPersistence:
    ...
    def keys(self, *, prefix=""):
        return tuple(sorted(key for key in self.state if key.startswith(prefix)))
...
def _prepare_route_handler(monkeypatch, *, families=("xor",), **overrides):
    ...
    monkeypatch.setattr(
        sys.modules[__name__], "_synthetic_ast_converter", lambda _ins: candidate
    )
    monkeypatch.setattr(
        handler._host,
        "rebuild",
        lambda *_args, **_kwargs: NativeMbaReconstruction(candidate, object()),
        raising=False,
    )
    monkeypatch.setattr(
        handler,
    ...
    # Keep the production catalogue available to saturation; disable only the
    # live direct-catalogue branch under test.
...
def test_direct_catalogue_hit_bypasses_replay_and_runtime(monkeypatch):
    ...
    runtime_calls = []
    monkeypatch.setattr(
        handler, "_create_composite_cache", lambda: cache_calls.append(1)
    )
    monkeypatch.setattr(
        handler, "_create_egglog_runtime", lambda: runtime_calls.append(1)
    )
...
def test_replay_hit_rebinds_current_terms_and_never_runs_fresh(monkeypatch):
    ...
    store = {}
    cache = EgglogIdbCompositeCache(_DictPersistence(store))
    cache.store(rewrite)
    ...
    assert (
        outcome.metadata["native_profile"]["fingerprint"] == receipt.island_fingerprint
    )
    assert (
        select_native_capture_profile((handler,)).fingerprint
...
class TestRealNativeLearnedReplay:
    binary_name = "libobfuscated.dll"

    def test_replay_uses_host_rebuild_and_proof_before_outer_mutation_acceptance(
        self,
        libobfuscated_setup,
        monkeypatch,
    ):
        """Exercise fresh learning, replay, native proof, and owner finalization."""

        del libobfuscated_setup
        families = ("add", "and", "bnot", "mul", "neg", "or", "sub", "xor")
        handler = _configured_live_handler(
            families=list(families),
            learned_replay_enabled=True,
            max_leaves=8,
            max_operator_nodes=32,
            max_degree=1,
            saturation_rounds=2,
            max_eclasses=256,
            max_enodes=512,
        ...
        real_host = native_mba_host_services()
        handler._host = real_host
        # The direct route is deliberately bypassed so the first call learns a
        # replay template through the real bounded extension saturation path.
        handler._direct_native_application = lambda **_kwargs: None
        ...
        for function_ea in idautils.Functions():
            ...
            for block_serial in range(mba.qty):
                ...
                instruction = block.head
                while instruction is not None:
                    candidate = real_host.capture_instruction(instruction)
                    ...
                    try:
                        ...
                        continue
                    if any(
                        term_cost(replacement) < term_cost(candidate.term)
                        and (
                            reconstruction := real_host.rebuild(
                                candidate,
                                replacement,
                            )
                        )
                        ...
                        for _rule, replacement, _index in applications
                    ):
                        site = (mba, block, instruction)
                        break
                    instruction = instruction.next
                if site is not None:
                    break
            if site is not None:
                break
        assert site is not None, "fixture database has no native replayable MBA site"
        _mba, block, instruction = site

        optimizer = PeepholeOptimizer(handler.maturities, OptimizationStatistics())
        assert optimizer.add_rule(handler)
        handler.begin_provider_outcome_capture()
        try:
            first = optimizer.get_optimized_instruction(block, instruction)
            assert first is not None
            assert getattr(first, "opcode", None) is not None
            first_receipt = handler.last_extraction_receipt
            assert first_receipt is not None
            assert first_receipt.execution_path == "fresh_saturation"
            assert first_receipt.skip_reason is None
            assert first_receipt.legacy_proof_verdict is True
            assert handler.provider_outcomes()[0].status.value == "improved"

            # Only the outer optimizer may finalize an accepted candidate; no
            # test-side instruction append or swap is used here.
            optimizer.record_mutation_accepted()
            assert handler.provider_outcomes()[0].status.value == "applied"
            assert handler._pending_composite_rewrite is None
            semantics = handler._current_replay_semantics()
            current_candidate = real_host.capture_instruction(instruction)
            assert current_candidate is not None
            canonical_current = canonicalize_mba_term(
                current_candidate.term
            ).canonical_term
            bucket_key = (
            ...
            second_receipt = handler.last_extraction_receipt
            assert second_receipt is not None
            assert second_receipt.execution_path == "learned_replay", second_receipt
            assert second_receipt.cache_status == "hit"
            assert second_receipt.replay_rebuild_elapsed_ms is not None
            assert second_receipt.replay_proof_elapsed_ms is not None
            assert second_receipt.legacy_proof_verdict is True
            assert handler.provider_outcomes()[1].status.value == "improved"
            optimizer.record_mutation_accepted()
            assert handler.provider_outcomes()[1].status.value == "applied"

            def raising_proof(*_args, **_kwargs):
                raise RuntimeError("native proof unavailable")

            monkeypatch.setattr(handler._host, "prove", raising_proof)
            failed = optimizer.get_optimized_instruction(block, instruction)
            assert failed is None
            failed_receipt = handler.last_extraction_receipt
            assert failed_receipt is not None
            assert failed_receipt.skip_reason is EgraphSkipReason.PROOF_FAILED
...
def test_cross_block_prepared_route_replays_before_fresh_saturation(monkeypatch):
    ...
    cache = EgglogIdbCompositeCache(_DictPersistence({}))
    cache.store(rewrite)
    handler._composite_cache = cache
    monkeypatch.setattr(
        handler,
...
def test_replay_semantics_snapshot_is_built_once_after_final_configuration(
    ...
    import d810_egglog.rules.egglog_optimizer as optimizer_module

    calls = []
    real_snapshot = optimizer_module.build_certified_catalogue_snapshot
...
def test_real_cache_corruption_is_reported_as_malformed_before_fresh(monkeypatch):
    ...
    handler._composite_cache = EgglogIdbCompositeCache(_DictPersistence(store))
    fresh_calls = []
    monkeypatch.setattr(
        handler,
        "_run_fresh_saturation",
    ...
    if failure == "rebuild":
        ...
        def fail_replay_rebuild(*_args, **_kwargs):
            if not rebuild_calls:
                ...
                return None
            return NativeMbaReconstruction(_candidate, object())
...
def test_rejected_or_next_attempt_clears_pending_and_storage_failure_is_safe(
    monkeypatch,
):
    handler, _candidate, source, replacement = _prepare_route_handler(
        monkeypatch,
...
def test_live_handler_admits_and_extracts_real_degree_two_boolean_candidate():
    ...
    host = native_mba_host_services()
    native_candidate = host.capture_ast(candidate, destination_size=4)
    assert handler._native_view_skip_reason(native_candidate.profile) is None
    ...
    replacement_ast = host.rebuild_ast(
        native_candidate,
        extraction.replacement_term,
    )
...
def test_live_handler_unsupported_root_replaces_prior_invocation_metadata(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "_synthetic_ast_converter",
        lambda _ins: capture_calls.append(_ins),
    )
    monkeypatch.setattr(
        handler._host,
        "prove",
    ...
    receipt = handler.last_extraction_receipt
    assert receipt is not None
    assert receipt.skip_reason is EgraphSkipReason.NON_MBA_CANDIDATE
    with pytest.raises(FrozenInstanceError):
    ...
    assert capture_calls == [unsupported]
    assert proof_calls == []
...
def test_live_handler_preflight_exception_preserves_lowered_profile(monkeypatch):
    ...
    monkeypatch.setattr(
        sys.modules[__name__],
        "_synthetic_ast_converter",
        lambda _ins: _direct_add_candidate(),
    )
    monkeypatch.setattr(
        handler,
...
def test_live_handler_direct_catalogue_proves_before_mop_without_egglog(
    ...
    monkeypatch.setattr(
        handler._host,
        "rebuild",
        lambda *_args, **_kwargs: NativeMbaReconstruction(
            candidate,
            _ReplacementInstruction(0x401000),
        ),
        raising=False,
    )
    monkeypatch.setattr(
...
def test_shadow_proves_real_native_terms_in_active_ast_runtime():
    ...
    host = native_mba_host_services()
    native_candidate = host.capture_ast(candidate, destination_size=4)
    reconstruction = NativeMbaReconstruction(replacement, object())
...
def test_live_handler_records_extractor_unavailability_for_supported_candidate(
    ...
    handler = _configured_live_handler()
    _disable_direct_route(handler)
    receipt = EgraphExtractionReceipt(skip_reason=EgraphSkipReason.RUNTIME_UNAVAILABLE)
    calls = []
    log_calls = []
...
def test_live_handler_opt_in_stage_timings_publish_after_reconstruction(monkeypatch):
    ...
    events = []
    original_canonical_applications = CanonicalMbaRuleCatalogue.canonical_applications
    original_finish_stage = handler._finish_stage

    def observed_canonical_applications(*args, **kwargs):
        events.append("native_match")
        return original_canonical_applications(*args, **kwargs)
    ...
    monkeypatch.setattr(
        ...
        observed_canonical_applications,
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: (
            events.append("native_z3") or True,
            "rule",
            None,
            True,
            True,
    ...
    monkeypatch.setattr(
        handler._host,
        "rebuild",
        lambda *_args, **_kwargs: NativeMbaReconstruction(object(), object()),
        raising=False,
    )
